...
# All data files are stored within "data" folder ./data
...
# which vessels in `vessel_assess` are listed in `iuu_list_current` match by IMO
...
peru_fishves_2021_sum <- peru_fishves_2021 %>% 
  ...
  summarise(n_foreign = sum(flag != "PER"), # n foreign flag observations
...
graph_top_10 <- top_10_prop %>% 
  ...
  labs(x = "Proportion of All Visits by Foreign Fishing Vessel",
       title = "Peruvian Ports with Highest Foreign Fishing Vessel Visits - 2021",
       subtitle = "(Ports with over 10 visits)")
