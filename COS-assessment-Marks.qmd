...
knitr::kable(iuu_vessels, 
             align = "lll", 
             caption = "Global Fishing Watch Vessels Currently Listed as IUU Vessels",
...
```{r question_2}
# paper assessing FOC (flags of convenience) - https://doi.org/10.1016/j.marpol.2020.103937

# vessel_assess

# scrape table from wikipedia
wiki_foc <- read_html("https://en.wikipedia.org/wiki/Flag_of_convenience") %>% 
  html_node(xpath = '/html/body/div[3]/div[3]/div[5]/div[1]/table[2]') %>%
  html_table(fill = TRUE) 
# rename column names
names(wiki_foc) <- c("Flag_state", "Ships_registered", "Bulk_carriers", "Container_ships", "General_cargo", "Oil_tankers", "other")

# scrape table from ITF Seafarers
